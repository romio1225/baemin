...
WISE.libs.Dashboard.DatasetDesigner = function() {
    ...
    this.render = function(props) {
    	self.menuConfig = userJsonObject.menuconfig.Menu;
        self.setState(props);
                                            ...
                                            '<div class="column" style="height: 55px; padding-bottom: 0;">' +
                                                '<div class="modal-article">' + 
                                                    '<div class="modal-tit">' +
                                                        ...
                                                        //20210318 AJKIM 데이터 원본 기준 인메모리 추가 dogfoot
                                                        (self.state.dataset.DATASET_TYPE == 'DataSetDs' && self.state.dataset.JOIN_TYPE != 'Y' && self.menuConfig.IN_MEMORY?
                                                        	'<div id="inMemoryCheck" style="margin-right:10px"></div>' +
                                                        	'<a id="dsFileUploadBtn" class="btn crud neutral">파일 업로드</a>': '') +
                                                        // 20211102 AJKIM 필드 제외 데이터 집합 수정 추가 dogfoot
                                                        (self.state.dataset.DATASET_TYPE == 'DataSetSQL' && self.menuConfig.MODIFY_EXCEPT_FIELD?
    ...
    //20220111 AJKIM 자산관리공사 필터 바 메시지 추가 기능 개발 dogfoot
    function onMessageParamCreate() {
    	var newParams = _.clone(self.state.params);
    	
    	var index = getNewMessageParamIndex();
    	
    	newParams.push({
            DS_ID: self.state.datasource.ORG_DS_ID || self.state.datasource.DS_ID,
            UNI_NM: "@WISE_MESSAGE_PARAM"+index,
            PARAM_NM: "@WISE_MESSAGE_PARAM"+index,
            PARAM_CAPTION: "WISE_MESSAGE_PARAM"+index,
            PARAM_TYPE: 'MESSAGE',
            COLOR: "#000000",
            FONT_SIZE: 14,
            MESSAGE_CONTENT: '',
            ORDER: newParams.length+1,
            WIDTH: 300,
            VISIBLE: 'Y',
            DATA_TYPE: '',
            CAPTION_WIDTH: 86.5,
    	...
    	var paramNameList = newParams.map(function(param){return param.PARAM_NM});
    ...
    function onQueryParamSearch() {
        ...
        for (var i = 0; i < self.state.params.length; i++) {
            if (self.state.params[i] && self.state.params[i].PARAM_TYPE == 'MESSAGE') {
            	newParams.push(_.clone(self.state.params[i]));
            }
        }
        	...
        	if(fieldSearch){
        		$.ajax({
                    ...
                    data: {
                        ...
                        //20220111 AJKIM 자산관리공사 필터 바 메시지 추가 기능 개발 dogfoot
                        PARAMS: JSON.stringify(self.utility.generateNewParamValues(self.state.params.filter(function(e){ if(e.PARAM_TYPE !== 'MESSAGE') return true;}))),
	...
	//20220111 AJKIM 자산관리공사 필터 바 메시지 추가 기능 개발 dogfoot
    function getNewMessageParamIndex(){
    	var params = _.clone(self.state.params);
    	var index = 1;

    	$.each(params, function(i, param){
    		if(param.PARAM_NM.indexOf("@WISE_MESSAGE_PARAM") > -1){
    			var tempIndex = param.PARAM_NM.slice(19) * 1;
    			if(index <= tempIndex){
    				index = tempIndex + 1;
    			}
    		}
        ...
        switch (self.state.dataset.DATASET_TYPE) {
            ...
            default:
                ...
                return $('#dsDesignerTableTree').dxTreeList({
                    height: 'calc(100% - 80px)',
                    dataSource: {
                    //20210328 AJKIM 파일 업로드 기능 추가 dogfoot
...
//                            if(fileList.constructor == Object || fileList.length == 0){
//                            	return $.ajax({
//                            		method: 'POST',
...
//                            }
                            ...
                            $.ajax({
                               ...
                               success : function(_data){
                            	   /*dogfoot 데이터 집합 디자이너>데이터항목의 칼럼 목록 중복 제거 처리 syjin 20230208*/
                               	//data = _data
                               	if(request === 'COLUMN'){
                             	   data = _data.reduce(function(acc, current) {
                        		    let value = acc.find(function(item){
                        		        return item.col_id === current.col_id;
                        		    })
                            ...
                            if(self.state.dataset.DATASET_TYPE == 'DataSetDs' && self.state.dataset.FILE_LIST){
                            	if(request == 'TABLE' || request == 'SEARCH'){
                                	$.each(self.state.dataset.FILE_LIST, function(i, file){
                                		if(file.tableName.toUpperCase().indexOf(search.toUpperCase()) >= 0){
                                			data.push({id: file.tableName, TBL_NM: file.tableName, TBL_CAPTION: "", text: file.tableName, parent: "", isDirectory: true, hasItems: true, isfile: true, TYPE: "TABLE"})
                                			data = data.concat(file.column);
                                		}
                                	});
                                	self.setState({tblList: data});
                                }
                                
                                if(data.length == 0 && request == 'COLUMN'){
                                	$.each(self.state.dataset.FILE_LIST, function(i, file){
                                		if(file.tableName == key)
                                			data = file.column;
                                	});
                                }
                            }
	...
	function dsMultiTblList() {
		...
		$.each(FROM_CLAUSE, function(i, d){
			$.each(self.state.dataset.SEL_CLAUSE, function(i, sel){
				var dsid = sel.DATASET_SRC;
				if(typeof dsid == "string" && dsid.indexOf(".") > -1){
					var temp = dsid.substring(dsid.length - 20, dsid.length);
					dsid = temp.split(".")[0];
					if(dsid.split("e").length > 1){
						dsid = dsid.split("e")[1];
					}
				}
				
				if(dsid == d.FK_DATASET_SRC){
					if(d.FK_TBL_NM == sel.TBL_NM){
						d.FK_DATASET_SRC = sel.DATASET_SRC;
					}
				}
				if(dsid == d.PK_DATASET_SRC){
					if(d.PK_TBL_NM == sel.TBL_NM){
						d.PK_DATASET_SRC = sel.DATASET_SRC;
					}
				}
			})
		})
		...
		$.each(rtnArr, function(i, data){
			$.each(self.state.dataset.FILE_LIST, function(j, file){
				if( data.tblnm == file.tableName){
    ...
    function dsInMemoryCheck(){
    	return $("#inMemoryCheck").dxCheckBox({
    		...
    		onContentReady: function(e){
    			if(self.state.dataset.FILE_LIST && (self.state.dataset.FILE_LIST.length > 0 || self.state.dataset.FILE_LIST.constructor == Object)){
    				onInMemoryChange(true);
    				e.component.option('disabled', true);
    				e.component.option('value', true);
    			}else if(self.state.dataset.JOIN_TYPE == 'Y'){
    				onInMemoryChange(true);
    				e.component.option('disabled', true);
    				e.component.option('value', true);
				...
				$('#ds_popup').dxPopup({
					...
					onShown: function(e){
						...
						if(e.component.option('title') === '업로드 파일 선택'){
						    WISE.alert('업로드 알림<br>업로드 할 파일은 첫번째 시트만 업로드 되고 1행에 헤더가 있어야 하며 .xls .xlsx .csv 파일만 가능합니다.<br>또한 파일 업로드시 자동으로 인메모리가 적용됩니다.');	
						}
						...
						$('#fileUploader').dxFileUploader({
							...
							uploadUrl: WISE.Constants.context + '/report/Upload/data.do?ckutf=false&&rename=true',
...
//											var datasetInfo = subjectInfos[indexId];
											...
											$.each(self.state.tblList, function(id, ds) {
												if (ds.TBL_NM.toUpperCase() === dataName.toUpperCase()) {
													sameDsName = true;
												}
											});
											
											if(sameDsName) {
												WISE.alert('현재 같은 이름의 테이블 명이 존재합니다.');
											} else {
												$("#ds_popup").dxPopup("instance").hide();
//												gProgressbar.show();

												var column = [];
												var fileUrl = data[0].realpath;
												
												$.each(data, function(index, col){
													column.push({
														TBL_NM: dataName,
														COL_NM: col.colNm,
														COL_CAPTION: col.colNm,
														DATA_TYPE: col.colType,
														LENGTH: col.colSize,
														PK_YN: "",
														TBL_CAPTION: "",
														TYPE: "COLUMN",
														hasItems: false,
														id: dataName+"-"+col.colNm.replace(/(\s*)/g, ""),
														isDirectory: false,
														parent: dataName,
														dataset_src: self.state.dataset.DATASRC_ID,
														text: col.colNm,
														fileUrl: fileUrl
													})
												});
												
												var fileList = self.state.dataset.FILE_LIST;
												if(!fileList){
													fileList = [];
												}
												fileList.push({
												...
												self.setState({FILE_LIST: fileList}, 'DATASET');
												
												 $('#dsDesignerTableTree').dxTreeList('instance').refresh();
												 $('#inMemoryCheck').dxCheckBox('instance').repaint();

											}
										} else {
											WISE.alert('테이블 명을 입력해주세요.');
										}
									}
								});
							}
        ...
        switch (self.state.dataset.DATASET_TYPE) {
            case 'DataSetSQL':
                self.queryDesigner.render({ 
                    ...
                    //20220111 AJKIM 자산관리공사 필터 바 메시지 추가 기능 개발 dogfoot
                    onMessageParamCreate: userJsonObject.siteNm == 'KAMKO'? onMessageParamCreate : undefined,
